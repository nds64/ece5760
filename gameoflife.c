...
int main(void)
{
  ...
  printf("Enter x Minimum: ");
  scanf("%d", &x_min);
  printf("%d\n", x_min);
  temp = 0;
  temp = x_min; // 2^23;
  temp |= 1 << 31;  //set bit 31 to 1
  //temp &= ~(1 << 30);  //set bit 30 to 0
  *h2p_lw_pio_addr = temp;// 2^23;
  ...
  {     
	...
	//temp = mp_x; 
	//temp |= 1 << 31;  //set bit 31 to 1
    //*h2p_lw_pio_addr = temp;
    //temp = 0;
	//temp = mp_y;
	//temp |= 1 << 30;  //set bit 30 to 1
    //*h2p_lw_pio_addr = temp;
    ...
    mp_x = mp_x + x;
    mp_y = mp_y - y;
    printf("x=%d, y=%d, left=%d, middle=%d, right=%d, dataSent=%x\n", mp_x, mp_y, left_click, middle_click, right_click, temp);
    //boundry condition
    if (mp_x < 2)
      mp_x = 2;
    if (mp_x > 630)
    ...
    *h2p_lw_pio_addr = temp;
	...
	VGA_PIXEL(mp_x+2,mp_y,0xff);					
		...
		// sprintf(num_string, "# = %d     ", total_count);
		 sprintf(time_string, "T=%3.0fmS gen=%d  ", elapsedTime, total_count);
